...
def caruana_ensemble(list_of_pipelines,X,y,dataset):
    ...
    outer_cv = StratifiedKFold(n_splits=10, random_state=12345)    
    fold_counter = 0
    ...
    for i, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)): 
        ...
        clfs_and_all_pred_val_ = [] 
        
        ##############  10% of the original dataset
        X_train_red, X_val_red, y_train_red, y_val_red = train_test_split(X_train_g, y_train_g, test_size = 0.02, train_size = 0.08, random_state=12345)   
        
        ##############  Test the pipes based on a 10% of the original dataset            
        t0 = time()
        
        for p_ in list_of_pipelines:
            
            counter, _, pipe_ = p_
               
            classifier_ = zero_training_classifier(pipe_, X_train_red, y_train_red)
            
            if classifier_ == None:
                print("This pipe did not pass the pynisher of 10%")
                pass
            
            else:
                clfs_and_all_pred_val_.append((counter, classifier_))
              
        filtered_clfs = clfs_and_all_pred_val_.copy()

        ############## Train and test in validation the pipelines that succed their training using pynisher of 10%
        final_time_t0 = time() - t0
        
        X_train, X_val, y_train, y_val = train_test_split(X_train_g, y_train_g, test_size = 0.2, random_state=12345)
        ...
        ### VALIDATION TO TRAIN ONLY ONCE A PIPELINE WITH MULTIPLE REPETITIONS IN THE ENSEMBLE
 
        ids_unique_pipes = []
        prev_value =  None
        print("--Initial len of small ensemble: ", len(filtered_clfs))
                
        for i in filtered_clfs:
                    
            counter_, pipe = i
            

            if prev_value is None:  ### crear list con ids (counter) unicos y evaluados   
                ...
                ####### train and test pipelines in train and validation sets
                    ...
                    prev_value = counter_ 
                    ids_unique_pipes.append(prev_value)
                ...
                if prev_value in ids_unique_pipes:
                    print("Yes, this pipe is repeated")
                    pass
        ...
        print("--Final len of small ensemble: ", len(clfs_and_all_pred_val))
        ...
        times_all_folds.append((final_time_t0, final_time_t1, final_time_t2, final_time_t3))
...
@pynisher.enforce_limits(wall_time_in_s=36)
...
def main(dataset_name):
	...
	#try:
		...
		for repetition in range(1,2):
			...
			print("avg x fold t0: ",statistics.mean(t0))
