...
class AutoEn:
    def __init__(self):
        ...
        self.__predictions_train = None
        self.__predictions_test = None
        self.__predictions_validation = None
        self.__best_prediction_in_validation = None
        
    list_ps = []
    final_voting_classifier = None
    final_ensemble = []
    predictions = None
    __X_train = None
    __y_train = None
    ...
    def validate_pipelines(pipelines_list, X, y):
        ...
        for pipe in pipelines_list:
            try:
                ...
                AutoEn.predictions = predictions
                counter = counter + 1
    ...
    for repetition in range(1, 2):
        ...
        for i in suggestions:
            ...
            list_ps.append(pipeline_steps)
    #########################################################################
    
    def caruana_ensemble(list_of_pipelines, X, y):
        ...
        for i, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
            ...
            AutoEn.__X_train_g = X_train_g
            AutoEn.__y_train_g = y_train_g
        ...
        AutoEn.__X_train = X_train
        AutoEn.__y_train = y_train
        classifiers_predictions_in_test = []
        clfs_and_all_pred_val = []
        t1 = time()
        for i in list_of_pipelines:
            counter_, _, pipe = i
            classifier = AutoEn.first_training_classifier(pipe, X_train, y_train)
            if classifier == None:
                pass
            ...
            for i in final_ensemble:
                ...
                if prev_value_ is None:
                    ...
                    pipe_2 = AutoEn.second_training_classifier(
                        pipeline, X_train_g, y_train_g)
                    AutoEn.final_ensemble.append(pipe_2)
        ...
        AutoEn.__X_train = X_train
        AutoEn.__pipeline = pipeline
        AutoEn.__y_train = y_train
        AutoEn.final_ensemble = final_ensemble
        return ensemble_results_in_test, ensemble_results_in_val, times_all_folds, time_caruana_ensemble_function, predictions_train, predictions_test, predictions_validation ,best_prediction_in_validation

    def fit(self,X,y):
        list_of_times = []
        AutoEn.list_ps = AutoEn.validate_pipelines(AutoEn.list_ps, X, y)
        time_building_validate_pipelines = time() 
        ensemble_results_in_test, ensemble_results_in_val, times_all_folds ,time_caruana_ensemble_function, self.__predictions_train, self.__predictions_test, self.__predictions_validation, self.__best_prediction_in_validation = AutoEn.caruana_ensemble(
            AutoEn.list_ps, X, y)
        final_time = time() #- t
        final_ensemble3 = list()
        counter =0 
        for i in self.final_ensemble:
                ...
                pipe_2 = AutoEn.second_training_classifier(
                        pipeline, AutoEn.__X_train_g, AutoEn.__y_train_g)
                print(pipe_2)
                print(pipe_2.steps[0])
                print(pipe_2.steps[0][0])
                print(pipe_2.steps[0][1])
                print('--------------------------------------------------')
                #El voting classifier solo acepta estimadores con un nombre unico
                #He optado por añadirle un numero a la derecha para que sean todos unicos
                #como son tuplas lo que he hecho es tupla --> lista --> tupla para poder cambiar el valor
                pipeline_list = list(pipe_2.steps[0])
                print(pipeline_list)
                print(pipeline_list[0])
                
                print(pipeline_list[1])
                print(pipeline_list[0])
                pipeline_list[0] = (pipeline_list[0]) + str(counter)
                
                pipeline_tuple = tuple(pipeline_list)
                final_ensemble3.append(pipeline_tuple)
                counter =counter +1
    
        final_voting_classifier = VotingClassifier(estimators=final_ensemble3, voting='soft')
        #final_voting_classifier.fit(AutoEn.__X_train_g, AutoEn.__y_train_g)
        final_voting_classifier.fit(X,y)
        self.final_voting_classifier = final_voting_classifier
       
        print(final_voting_classifier)
        return self.final_voting_classifier
      
    def predict(self, XX):
        predict = self.final_voting_classifier.predict(XX)
        return predict

    def predict_proba(self, XX):
        predict = self.final_voting_classifier._predict_proba(XX)
        return predict
